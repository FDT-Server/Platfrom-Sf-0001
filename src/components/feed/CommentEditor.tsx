...
export default function CommentEditor({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (isReadOnly) {
      toast.error("Read-Only Mode: You cannot comment.");
      return;
    }
    if (!text.trim()) return;
    ...
    <form onSubmit={handleSubmit} className="relative flex gap-3 bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
      ...
      <div className="flex-1 flex flex-col gap-2 min-w-0">
        <textarea
          ...
          placeholder={isReadOnly ? "Read-Only Mode active." : placeholder}
          value={text}
          onChange={(e) => setText(e.target.value)}
          disabled={isReadOnly}
          className="w-full text-xs font-medium bg-slate-50 border border-slate-200/80 rounded-xl p-3 text-slate-800 placeholder:text-slate-400 focus:outline-none focus:border-blue-500 transition-colors shadow-inner resize-none disabled:opacity-50 disabled:cursor-not-allowed"
        />
